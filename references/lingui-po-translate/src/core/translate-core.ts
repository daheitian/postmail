...
import {
  hasManualMarking,
  shouldSkipForManual,
} from "../file-formats/po/comment-parser";
  ...
  inputs.forEach((value, key) => {
    ...
    if (shouldSkipForManual(parsed, args.targetLng)) {
      ...
      const translationStatus = targetTranslation
        ...
        : "not yet translated";
      console.info(
        `@manual:${args.targetLng} - skip "${key}" (${translationStatus})`,
      );
      return;
    }
...
async function readOverrideSource(
  ...
  try {
    ...
    console.warn(
      `Warning: Could not read override source file ${overridePath}`,
    );
    return null;
  }
}
...
export async function translateCore(args: CoreArgs): Promise<CoreResults> {
  ...
  if (filtered.toSkip.size > 0) {
    console.info(
      `Skip ${filtered.toSkip.size} entries marked as @manual:${args.targetLng}`,
    );
  }
  if (filtered.toCopyOriginal.size > 0) {
    console.info(
      `Copy original text for ${filtered.toCopyOriginal.size} entries with @manual (no override)`,
    );
  }
  if (filtered.toTranslateFromOverride.size > 0) {
    const overrideLng = args.sourceOverride.get(args.targetLng);
    console.info(
      `Translate ${filtered.toTranslateFromOverride.size} entries from override source ${overrideLng}`,
    );
  }
  ...
  if (filtered.toTranslate.size >= 1) {
    serviceInvocation = await invokeTranslationService(
      filtered.toTranslate,
      args,
    );
  }
  ...
  if (filtered.toTranslateFromOverride.size > 0) {
    const overrideLng = args.sourceOverride.get(args.targetLng);
    if (overrideLng) {
      const overrideSource = await readOverrideSource(
        args.srcFile,
        overrideLng,
      );
      if (overrideSource) {
        ...
        if (overrideInputs.size > 0) {
          ...
          const overrideResult = await invokeTranslationService(
            overrideInputs,
            overrideArgs,
          );
