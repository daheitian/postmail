...
function activate(context) {
  ...
  try {
    ...
    nativeSnippetLanguages = packageJson.contributes.snippets.map(
      (s) => s.language,
    );
  } catch (error) {
    console.error(
      "Failed to load package.json for native snippet languages:",
      error,
    );
  ...
  const configWatcher = vscode.workspace.onDidChangeConfiguration((event) => {
    if (event.affectsConfiguration("datastar.enabledLanguages")) {
      ...
      vscode.window.showInformationMessage(
        "Datastar language settings updated!",
      );
    }
  ...
  try {
    ...
    const validCustomAttributes = customAttributes.filter((attr) => {
      if (typeof attr !== "string" || !/^[a-z][a-z0-9-]*$/.test(attr)) {
        console.warn(
          `Invalid custom attribute name: ${attr}. Must be lowercase with hyphens.`,
        );
        return false;
      }
      return true;
    ...
    console.log(
      `Datastar grammar updated with ${allAttrs.length} attrs (${BUILTIN_ATTRIBUTES.length} built-in + ${validCustomAttributes.length} custom)`,
    );
  } catch (error) {
    console.error("Failed to generate Datastar grammar:", error);
    vscode.window.showErrorMessage(
      `Failed to update Datastar grammar: ${error.message}`,
    );
  }
}
