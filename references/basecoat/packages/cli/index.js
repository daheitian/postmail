...
async function getAvailableComponents() {
  try {
    ...
    console.error(
      "Error reading component source directories from CLI assets:",
      error,
    );
    return [];
  }
...
async function ensureConfiguration() {
  ...
  if (
    !config.templateDest ||
    config.templateDest === DEFAULT_CONFIG.templateDest
  ) {
    pathPrompts.push({
      type: "input",
      name: "templateDest",
...
async function addComponent(componentName) {
  ...
  const assetsDir = path.resolve(cliScriptRunningDir, "assets");
  const templateExt =
    config.templateEngine === "jinja" ? ".html.jinja" : ".njk";

  const templateSource = path.join(
    assetsDir,
    config.templateEngine,
    `${componentName}${templateExt}`,
  );
  const scriptSource = path.join(assetsDir, "js", `${componentName}.js`);

  const templateDestPath = path.resolve(
    process.cwd(),
    config.templateDest,
    `${componentName}${templateExt}`,
  );
  const scriptDestPath = path.resolve(
    process.cwd(),
    config.scriptDest,
    `${componentName}.js`,
  );
  ...
  try {
    ...
    if (!templateExists) {
      console.error(
        `  Error: Template file for component '${componentName}' not found in CLI assets. Searched: ${templateSource}`,
      );
      // Optionally, list available template engines or files for that component if helpful
    }
    if (!scriptExists) {
      console.error(
        `  Error: Script file for component '${componentName}' not found in CLI assets. Searched: ${scriptSource}`,
      );
    }
...
program
  ...
  .argument(
    "[components...]",
    "Names of components to add (e.g., dialog select)",
  )
    ...
    try {
      if (!componentsArg || componentsArg.length === 0) {
        ...
        if (availableComponents.length === 0) {
          console.error(
            "Error: No components available in the CLI. Build might be corrupted.",
          );
          return;
        }
        ...
        componentsToAdd = Array.isArray(componentsArg)
          ? componentsArg
          : [componentsArg];
      }
      ...
      if (error.isTtyError || error.constructor?.name === "ExitPromptError") {
        // Handle inquirer cancellation
        console.log("\nOperation cancelled by user.");
      } else {
        console.error(
          "\nAn unexpected error occurred during the add command:",
          error,
        );
      }
