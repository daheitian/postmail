...
- 一次性从别的博客或 CMS 导入内容：把当前站点的 `/skill.md` 地址（例如 `https://example.com/skill.md`）交给 AI 助手——这份导入指南会绑定当前站点，并使用同一套 HTTP API。详见 [导出与导入](export-and-import.md#从别的博客或-cms-迁移过来)。
