...
> **Important:** This is separate from the `AUTH_SECRET` in `.dev.vars` (which is for local development only). Do not change the production secret after your site is live — it will invalidate all sessions. If you get locked out, use `pnpm run reset-password` to generate a password reset link.
...
| `pnpm run db:migrate:remote` | Apply database migrations (remote) |
