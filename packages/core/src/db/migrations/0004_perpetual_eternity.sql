...
CREATE UNIQUE INDEX `uq_upload_session_media_id` ON `upload_session` (`media_id`);--> statement-breakpoint
CREATE UNIQUE INDEX `uq_upload_session_temp_storage_key` ON `upload_session` (`temp_storage_key`);--> statement-breakpoint
CREATE UNIQUE INDEX `uq_upload_session_final_storage_key` ON `upload_session` (`final_storage_key`);--> statement-breakpoint
CREATE INDEX `idx_upload_session_site_state` ON `upload_session` (`site_id`,`state`);--> statement-breakpoint
CREATE INDEX `idx_upload_session_site_expires_at` ON `upload_session` (`site_id`,`expires_at`);