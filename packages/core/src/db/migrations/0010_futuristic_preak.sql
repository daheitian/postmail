...
CREATE TABLE `__new_nav_item` (
	...
	CONSTRAINT "chk_nav_item_system_key" CHECK("__new_nav_item"."system_key" IS NULL OR "__new_nav_item"."system_key" IN ('latest', 'featured', 'collections', 'archive', 'rss', 'settings')),
...
INSERT INTO `__new_nav_item`("id", "site_id", "type", "system_key", "label", "url", "placement", "position", "created_at", "updated_at") SELECT "id", "site_id", "type", "system_key", "label", "url", "placement", "position", "created_at", "updated_at" FROM `nav_item`;--> statement-breakpoint
...
CREATE UNIQUE INDEX `uq_nav_item_site_system_key` ON `nav_item` (`site_id`,`system_key`) WHERE "nav_item"."system_key" IS NOT NULL;