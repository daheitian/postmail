-- Rebuild the pre-1.0 SQLite schema around site-aware core primitives.
-- This migration is intentionally destructive: existing local data is dropped.
...
--> statement-breakpoint
DROP TABLE IF EXISTS collection_directory_item;
--> statement-breakpoint
...
CREATE INDEX `idx_api_token_site_id` ON `api_token` (`site_id`);--> statement-breakpoint
...
--> statement-breakpoint
CREATE INDEX `idx_collection_site_created_at` ON `collection` (`site_id`,`created_at`);--> statement-breakpoint
CREATE TABLE `media` (
	`id` text PRIMARY KEY NOT NULL,
	`site_id` text NOT NULL,
	`post_id` text,
...
--> statement-breakpoint
CREATE INDEX `idx_media_site_post_id_position` ON `media` (`site_id`,`post_id`,`position`);--> statement-breakpoint
CREATE UNIQUE INDEX `uq_media_site_post_position` ON `media` (`site_id`,`post_id`,`position`) WHERE "media"."post_id" IS NOT NULL;--> statement-breakpoint
CREATE UNIQUE INDEX `uq_media_provider_storage_key` ON `media` (`provider`,`storage_key`);--> statement-breakpoint
CREATE INDEX `idx_media_site_media_kind_post_id` ON `media` (`site_id`,`media_kind`,`post_id`);--> statement-breakpoint
...
CREATE TABLE `post_collection` (
	...
	`created_at` integer NOT NULL,
	PRIMARY KEY(`site_id`, `post_id`, `collection_id`),
	FOREIGN KEY (`site_id`) REFERENCES `site`(`id`) ON UPDATE no action ON DELETE cascade,
	FOREIGN KEY (`post_id`) REFERENCES `post`(`id`) ON UPDATE no action ON DELETE cascade,
	FOREIGN KEY (`collection_id`) REFERENCES `collection`(`id`) ON UPDATE no action ON DELETE cascade
);
--> statement-breakpoint
CREATE INDEX `idx_post_collection_site_collection_id` ON `post_collection` (`site_id`,`collection_id`);--> statement-breakpoint
CREATE INDEX `idx_post_collection_site_collection_created_post` ON `post_collection` (`site_id`,`collection_id`,`created_at`,`post_id`);--> statement-breakpoint
CREATE TABLE `post` (
	`id` text PRIMARY KEY NOT NULL,
	`site_id` text NOT NULL,
	...
	`updated_at` integer NOT NULL,
	FOREIGN KEY (`site_id`) REFERENCES `site`(`id`) ON UPDATE no action ON DELETE cascade,
	FOREIGN KEY (`site_id`,`reply_to_id`) REFERENCES `post`(`site_id`,`id`) ON UPDATE no action ON DELETE no action,
	FOREIGN KEY (`site_id`,`thread_id`) REFERENCES `post`(`site_id`,`id`) ON UPDATE no action ON DELETE no action,
...
--> statement-breakpoint
CREATE UNIQUE INDEX `uq_post_site_id_id` ON `post` (`site_id`,`id`);--> statement-breakpoint
CREATE INDEX `idx_post_site_thread_id` ON `post` (`site_id`,`thread_id`);--> statement-breakpoint
CREATE INDEX `idx_post_site_thread_live_created` ON `post` (`site_id`,`thread_id`,`created_at`,`id`) WHERE "post"."deleted_at" IS NULL;--> statement-breakpoint
CREATE INDEX `idx_post_site_status_deleted_published` ON `post` (`site_id`,`status`,`deleted_at`,`published_at`);--> statement-breakpoint
CREATE INDEX `idx_post_site_status_deleted_activity` ON `post` (`site_id`,`status`,`deleted_at`,`last_activity_at`);--> statement-breakpoint
CREATE INDEX `idx_post_site_root_live_published_activity` ON `post` (`site_id`,`last_activity_at`,`id`) WHERE "post"."deleted_at" IS NULL AND "post"."reply_to_id" IS NULL AND "post"."status" = 'published';--> statement-breakpoint
CREATE INDEX `idx_post_site_root_live_draft_updated` ON `post` (`site_id`,`updated_at`,`id`) WHERE "post"."deleted_at" IS NULL AND "post"."reply_to_id" IS NULL AND "post"."status" = 'draft';--> statement-breakpoint
CREATE INDEX `idx_post_site_reply_live_thread_created` ON `post` (`site_id`,`thread_id`,`created_at`,`id`) WHERE "post"."deleted_at" IS NULL AND "post"."reply_to_id" IS NOT NULL AND "post"."status" = 'published';--> statement-breakpoint
CREATE INDEX `idx_post_site_featured_live_featured_at` ON `post` (`site_id`,`featured_at`,`thread_id`,`id`) WHERE "post"."deleted_at" IS NULL AND "post"."status" = 'published' AND "post"."featured_at" IS NOT NULL;--> statement-breakpoint
...
--> statement-breakpoint
CREATE INDEX `idx_site_setting_site_id` ON `site_setting` (`site_id`);--> statement-breakpoint
CREATE TABLE `site_domain` (
	...
	`redirect_to_primary` integer DEFAULT true NOT NULL,
	`created_at` integer NOT NULL,
	`updated_at` integer NOT NULL,
	FOREIGN KEY (`site_id`) REFERENCES `site`(`id`) ON UPDATE no action ON DELETE cascade,
...
--> statement-breakpoint
CREATE UNIQUE INDEX `uq_site_domain_host` ON `site_domain` (`host`);--> statement-breakpoint
CREATE INDEX `idx_site_domain_site_id` ON `site_domain` (`site_id`);--> statement-breakpoint
...
--> statement-breakpoint
CREATE INDEX `idx_site_member_user_id` ON `site_member` (`user_id`);--> statement-breakpoint
CREATE TABLE `site` (
	`id` text PRIMARY KEY NOT NULL,
	`key` text NOT NULL,
...
--> statement-breakpoint
CREATE UNIQUE INDEX `uq_site_key` ON `site` (`key`);--> statement-breakpoint
CREATE TABLE `user` (
	`id` text PRIMARY KEY NOT NULL,
	`name` text NOT NULL,
	`email` text NOT NULL,
