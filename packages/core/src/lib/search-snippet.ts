...
function sliceSnippetWindow(
  ...
  if (end - start > SNIPPET_MAX_LENGTH) {
    ...
    const trimStart = Math.min(start + Math.floor(overflow / 2), matchIndex);
    start = trimStart;
...
/**
 * Builds an HTML-safe search snippet for a set of plain-text fields.
 * Returns the first field that contains a query term, truncated around the
 * earliest match and wrapped with <mark> tags.
 */
