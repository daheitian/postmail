...
export const ArchivePage: FC<ArchivePageProps> = ({
  ...
  const baseUrl = filterQuery ? `/archive?${filterQuery}` : "/archive";
    ...
    <div>
      <header class="mb-8">
        ...
        <nav class="flex flex-wrap gap-2 mt-4">
          <a
            href="/archive"
            class={`badge ${!format ? "badge-primary" : "badge-outline"}`}
          >
            {t({
              message: "All",
                  ...
                  <article key={post.id} class="flex items-baseline gap-4">
                    ...
                    <div class="flex-1 min-w-0">
                      <a href={post.permalink} class="hover:underline">
                        {post.title || post.excerpt || `Post #${post.id}`}
                      </a>
                      {!format && (
